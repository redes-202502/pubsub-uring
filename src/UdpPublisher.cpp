...
int main(int argc, char **argv) {
  ...
  vector<byte> handshakeBuffer(encoder.sizeHandshakePub(opts.clientId));
  ...
  vector<byte> ackBuffer(RECV_BUFFER_SIZE);
  ...
  while (!STOP_REQUESTED) {
    const auto msgLen =
        genMsg.generateMessage(msgBuffer.data(), msgBuffer.size());
    ...
    iovec sendIov{};
    sendIov.iov_base = publishBuffer.data();
    sendIov.iov_len = publishBuffer.size();

    msghdr sendMsg{};
    sendMsg.msg_name = &serverAddr;
    sendMsg.msg_namelen = sizeof(serverAddr);
    ...
    io_uring_prep_sendmsg(sqe, sock, &sendMsg, 0);
    ...
    if (cqe->res < 0) {
      ...
      println(stderr, "\033[31mSend returned 0 bytes\033[0m");
      io_uring_cqe_seen(&ring, cqe);
      break;
    }
